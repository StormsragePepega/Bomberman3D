...
public class RandomSpawn : MonoBehaviour
{
    ...
    public void SpawnDestrBlocks()
    {
        ...
        M1:
            ...
            if(true ^ (tempDestrBlocks.x >= -8.5 & tempDestrBlocks.x <= -7.5 & tempDestrBlocks.z <= 2.5 & tempDestrBlocks.z >= 0.5)) {
                DestrBlocks.Add(tempDestrBlocks);
                colliders = Physics.OverlapBox(tempDestrBlocks, boxSize);
                if (colliders.Length == 0)
                {
                    Instantiate(destBlockPrefab, tempDestrBlocks, Quaternion.identity, DestrBlocksParent);
                }
                else
                {
                    goto M1;
